 ...
 *
 * This version includes the MAX17048 Fuel Gauge and critical low-power control
 * for the external boost converter (MCP1640) using GPIO8.
 *
 * FIXES/FEATURES IN THIS VERSION:
 * 1. Implemented Booster Control: GPIO8 (PIN_BOOSTER_EN) HIGH/LOW to enable/disable the MCP1640 boost converter.
 * 2. Integrated MAX17048 Fuel Gauge: Reads battery Voltage and SoC.
 * 3. Corrected E-Paper driver to GxEPD2_290_BS.
 * 4. Added RTC Stability Check.
 * 5. Display Battery info on E-Paper.
 * 6. **FIXED**: MAX1704X library method name is now cellPercent() for SoC.
 */
...
// Sleep duration: Set to 180 seconds (3 minutes)
uint64_t uS_TO_S_FACTOR = 1000000; 
uint64_t TIME_TO_SLEEP = 180; // 180 seconds (3 minutes)
...
void Read_Sensors_and_Power() { 
    ...
    // Read MAX17048 values (NEW - Corrected method names)
...
void displayReadings() {
    ...
    // Initialize the display here, right before drawing
    ...
    do {
        ...
        // Display Battery Data (NEW) - Placed below environmental data
...
void setup() {
    // 0. Setup Serial and check wake-up reason
    Serial.begin(115200);
    ...
    // ** POWER BOOSTER CONTROL SETUP ** (NEW)
    ...
    // 1b. Initialize MAX17048 (NEW - using MAX1704X library)
    ...
    // 2. Initialize SPI bus pins for SD/E-Paper
    ...
    // >>> DEEP SLEEP ENABLED <<< (Set to 180 seconds)
    display.hibernate();
